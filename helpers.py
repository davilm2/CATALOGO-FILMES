...
class FormularioFilme(FlaskForm):
    nome = StringField('Nome do Filme', [validators.data_required(), validators.Length(min=1, max = 50)])
    categoria = StringField('Categoria', [validators.data_required(), validators.Length(min=1, max = 40)])
    streaming = StringField('Streaming', [validators.data_required(), validators.Length(min=1, max = 20)])
...
def recupera_imagem(id):
    ...
    return 'question-mark.jpg'

def deleta_arquivo(id):
    arquivo = recupera_imagem(id)
    if arquivo != 'question-mark.jpg':
